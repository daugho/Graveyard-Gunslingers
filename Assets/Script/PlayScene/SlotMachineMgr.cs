...
public class SlotMachineMgr : MonoBehaviour
{
    ...
    IEnumerator StartSlot ( int SlotIndex )
    {
        ...
        int imageCount = 4; // 실제 슬롯 이미지 개수
        float cycleHeight = (imageHeight * imageCount)-200; // 총 높이 = 800
        for ( int i = 0 ; i < ( ItemCnt * ( 6 + SlotIndex * 4 ) + answer[SlotIndex] ) * 2 ; i++ )
        {
            SlotSkillObject[SlotIndex].transform.localPosition -= new Vector3(0, imageHeight, 0);

            if (SlotSkillObject[SlotIndex].transform.localPosition.y <= -cycleHeight + imageHeight)
            {
                SlotSkillObject[SlotIndex].transform.localPosition += new Vector3(0, cycleHeight, 0);
            }
